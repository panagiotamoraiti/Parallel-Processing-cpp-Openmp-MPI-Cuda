...
//	w = 3000;
    ...
    const char *input_rgb_path = "flower.raw";
    ...
    const char *output_feature_map_path = "feature_map_flower.raw";
    const char *output_feature_map_path_after_pooling = "feature_map_after_pooling_flower.raw";
	...
	if(rank == 0)
	{
	    ...
	    for (i = 0; i < h; ++i)
	    {
	        ...
	        {
	            int index = (i * w + j) * 3;
	            matrixR[i][j] = input_rgb_data[index];
	            matrixG[i][j] = input_rgb_data[index + 1];
	            matrixB[i][j] = input_rgb_data[index + 2];
	        }
	    }
...
//	}
    ...
    int count = h*(w-2*kernelCenter)/P;
    MPI_Gather(local_output, count, MPI_INT, result_parallel_1D, count, MPI_INT, 0, MPI_COMM_WORLD); // h=12, P=4
...
//					  result_serial, result_parallel, result_serial_final, result_parallel_final, h, w);
	    ...
	    char *feature_map = (char *)malloc(w * h);
		for (i = 0; i < h; ++i)
		{
		    for (j = 0; j < w; ++j)
		    {
		...
		char *feature_map_after_pooling = (char *)malloc(w/2 * h/2);
		for (i = 0; i < h/2; ++i)
		{
		    for (j = 0; j < w/2; ++j)
		    {
	    ...
	    writeRawFile(output_feature_map_path, feature_map, w * h);
	    writeRawFile(output_feature_map_path_after_pooling, feature_map_after_pooling, w/2 * h/2);
